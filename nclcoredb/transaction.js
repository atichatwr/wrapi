...
async function getDataTransaction(callback){
    ...
    let statement = `SELECT     TOP (100) PERCENT dbo.MFTS_Fund.Fund_Id, dbo.Fund_Cen_AllottedTransactions.TRANS_DATE, dbo.Fund_Cen_AllottedTransactions.AMC_CODE, dbo.Fund_Cen_AllottedTransactions.FILTER01, 
                    ...
                    dbo.Fund_Cen_AllottedTransactions.FILTER04, dbo.MFTS_Account.Ref_No
...
async function importDataTransaction(datarows, callback){
    for (const key in datarows) {
        ...
        try{
            ...
            let average_cost            = datarows[key].AVERAGE_COST
            ...
            const Fund_Id               = datarows[key].Fund_Id
            ...
            if(navprice     === undefined || navprice.length == 0)  {navprice = 0.000000}
            if(amountunit   === undefined || amountunit.length == 0 )  {amountunit = 0.000000}
            if(Amount_U   === undefined || Amount_U.length == 0 )  {Amount_U = 0.000000}
            if(average_cost === undefined)  {average_cost = 0.000000}
            if(unitbalance  === undefined)  {unitbalance = 0.000000}
            if(amountbaht   === undefined || amountbaht == 0)  {amountbaht = 0.000000}
            ...
            await new sql.ConnectionPool(config).connect().then(pool => {
                ...
                if (data_row == 1){
                    ...
                    WHERE MFTS_Transaction.FUNDCODEI =@FUNDCODEI  
                    AND UNITHOLDERI         =@UNITHOLDERI 
                    AND TRANSACTION_DATE    = '${TranDate}'
            ...
            let unit                   = TranStatus === "ALLOTTED" ?  allottedUnit :  amountunit  
