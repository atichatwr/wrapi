...
async function setCredentUpdate(  txid, new_status, xemail, cardId, DocNo, callback ) {
    let result = "ok";
    let approvel = "DipChip"
    let statusSendMail = "U";
    try 
    {
      var statement = `INSERT INTO Fund_Credent_Update ( txid, new_status, xemail, approvel, statusSendMail,cardNo, DocumentNo ) 
      ...
      await new sql.ConnectionPool(config).connect().then(pool => {
        ...
        .input("cardNo"         , sql.NVarChar, cardId)
        .input("DocumentNo"     , sql.NVarChar, DocNo)
        .query(statement);
...
//-----------------------best 
async function setCredentQuestionaire(datas){
  let carNo  
  let title 
  let name
  let last_Name
  let object = datas.section1
    for (const key in object) {
      if (Object.hasOwnProperty.call(object, key)) {
        // const element = object[key];
        console.log(object[key].order)
        console.log(object[key].question)
        console.log(object[key].answer)

      }
    }
  //console.log(data.section1)
    // let dataSec1 =  datas.section1
    // var querysec1 = [];
    // let text 
