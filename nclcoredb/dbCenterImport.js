...
import {selctDataOrderQuery ,importOrderQuery ,checkDataOrderInquiry} from './orderquery.js' 
...
async function importFundHoliday(datarows, callback) {
    try {
        ...
        let pool = await sql.connect(config);
        await pool.request().query(`DELETE FROM Fund_Cen_Fund_Holiday_Date WHERE timestampx >= '${timestampx}' `);
        pool.request().bulk(table, function(err, result) {
            if (err) {
                console.log(err);
            } else {
