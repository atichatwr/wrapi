...
async function selectDataAccountaddr(callback){
    try{
        let statement = `SELECT * FROM Fund_Cen_Address   WHERE     (cardNumber = '3760500954010')  AND (address_type = 'doc') `  //ID BETWEEN 2001 AND 3000 WHERE cardNumber  = '3100500416064' '1100600281300' WHERE cardNumber  = '3101201738958'
        await new sql.ConnectionPool(config).connect().then(pool => { 
            return pool.request()
            .query(statement); 
          }).then(result => {
    ...
    for (const key in datarows) {
        try{
            ...
            let statement = `INSERT INTO Account_Address (Cust_Code, Addr_Seq, Addr_No ,Place ,Road ,Tambon_Id ,Amphur_Id ,Province_Id
                ...
                // let pool = await sql.connect(config);
                // await pool.request().query(`DELETE FROM Account_Address WHERE Cust_Code = '${cardNumber}'AND Addr_Seq = ${seqAddr} `).then(result => {
                //     sql.close(); }).catch(err => {sql.close();})
                // console.log("icard :"+ cardNumber)
                ...
                await new sql.ConnectionPool(config).connect().then(pool => {   
                    ...
                    // .input("country",               sql.NVarChar(3),        country)
                    .query(statement)  
                }).then(result => {
                sql.close();
                // console.log(result);
...
async  function getnameaccount(cardname,callback){
    let statement = `SELECT  Title_Name_T, First_Name_T, Last_Name_T , Email  FROM Account_Info
...
await new sql.ConnectionPool(config).connect().then(pool => {
  ...
  let fullname ,email
  if (rowsAffected > 0){  
    fullname    =  " "+ result.recordset[0].First_Name_T +"  "+ result.recordset[0].Last_Name_T 
    email       = result.recordset[0].Email
}
  sql.close();
  return callback(fullname, email)
}).catch(err => {
  console.log(err);
  sql.close();
  return callback(err)
...
async function updatesendmail(cust_id){
    try{
...
export {SelectDataAccount ,selectDataAccountaddr ,checkAccountinfo, importDataAccountaddr ,callsendmail,updatesendmail ,callgroupsendmail,callsendsmail}