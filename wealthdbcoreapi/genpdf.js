...
async function genpdf(datarows ,callback){ 
    ...
    for (const key in datarows) {
        ...
        if(InvestMent.trim().length == 0){InvestMentX = 42; InvestMentY=186+16} 
       ...
       await getBankBranch(Bank_IdReturn,Bank_BranchReturn,(Brname)=>{nameBankBranchReturn =Brname})
        ...
        //     acountnamey2 = 0
        //     accountname2 = " "
        accountnamex = 486  - ( accountname.length * 2)
        acountnamey2 = 392
...
// doc.addPage()
// doc.pipe(fs.createWriteStream('Open_Account.pdf'));
// doc.pipe(res)

// doc.image('image/WR_NewForm_page_1.jpg', 320, 15, {fit: [100, 100]})
// .rect(320, 15, 100, 100)
// .stroke()
// .text('Fit', 320, 0);
...
async function pdfgroupshowDataform(callback){
    try{
        let statement = `SELECT  top(50)   dbo.Customer_send_form.send, 
                            dbo.Customer_send_form.cust_code, dbo.Account_Info.Title_Name_T, 
                            dbo.Account_Info.First_Name_T, dbo.Account_Info.Last_Name_T, 
                            dbo.Account_Info.Email
                        FROM dbo.Customer_send_form INNER JOIN
                            dbo.Account_Info ON 
                            dbo.Customer_send_form.cust_code = dbo.Account_Info.Cust_Code RIGHT
                            OUTER JOIN
                            dbo.VW_SUM_UNITHOLDER_VERIFY_SEND_PDF ON 
                            dbo.Customer_send_form.cust_code = dbo.VW_SUM_UNITHOLDER_VERIFY_SEND_PDF.Account_No
                        WHERE (dbo.Customer_send_form.send IS NULL) 
                         `
                        //  AND  dbo.Customer_send_form.cust_code = '3760500954079'
    await new sql.ConnectionPool(config).connect().then(pool => { 
            return pool.request() 
            .query(statement);
          }).then(result => {
...
// }
async function DeleteFileHoldingReport(callback){
    let DATA_FUND
    try {
        ...
        for (let key = 0; key < DATA_FUND.length; key++) {
        // console.log(DATA_FUND[key].Ref_1)
            let filePath = DATA_FUND[key].Ref_1.replace(/\\/g, '/') + '\\'+ DATA_FUND[key].Ref_2.replace(/\\/g, '/');
            ...
            if (err) {
              ...
              // File exists, perform the deletion
          ...
          fs.access(filePath, fs.constants.F_OK, (err) => {
            if (err) {
            //   console.error('File does not exist or is inaccessible:', err);
            //   console.log('UPDATE '+DATA_FUND[key].Ref_2)
              updateDataDeleteFileReport(DATA_FUND[key].Ref_2)
            } else {
              // File exists, perform the deletion
              fs.unlink(filePath, (err) => {
                if (err) {
...
async function updateDataDeleteFileReport(filename){
    try{
        const today                 = new Date()
        let date_ob                 = new Date(today);
        let date                    = ("0"+date_ob.getDate()).slice(-2); 
        ...
        await new sql.ConnectionPool(config).connect().then(pool => {
            ...
            .input("Modify_Date" ,sql.VarChar(20) ,timestampx)  
            .query(` UPDATE  Request_Report_log  SET ref_4 = 'true' , Modify_Date = @Modify_Date ,   Modify_User = '999'  WHERE (Ref_2 = @Filename)`)
        }).then(result => {   
...
export {genpdf,selectaccountToPdf,pdfshowDataform,pdfgroupshowDataform, selectaccountToPdfbyid, selectaccountsToPdf,DeleteFileHoldingReport}
