...
async function getRef(refcode,segno,callback){
  ...
  { 
      await new sql.ConnectionPool(config).connect().then(pool => {
          return pool.request()
          .input('GTran_No' , sql.Char(12), refcode)
          .execute(`[dbo].[USP_MFTS_Gen_Tran_No_2]`);
      }).then(result => {
          let rowsAffected = result.rowsAffected.toString()
          // if (rowsAffected > 0){   tranNo = result.returnValue  }
          sql.close();
        //  console.log(result.recordset[0].Tran_No )
          return callback(result.recordset[0].Tran_No)
      }).catch(err => {
          console.log(err);
          sql.close();
          return  callback(err)
