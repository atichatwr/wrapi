...
async function importOrderInquiry(datarows, callback) {
    ...
    try {
        ...
        await pool.request().query("DELETE FROM Fund_Cen_OrderInquiry");
        ...
        for ( let key in datarows) {
            //try{
                ...
                await new sql.ConnectionPool(config).connect().then(pool => {
                    ...
                    if (data_row > 0){  console.log("Insert  MFTS_UnitBalance  CODE " + datarows[key].FUND_CODE + " SUCEESS") } 
                    sql.close(); 
